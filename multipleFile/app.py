...
def main():
    ...
    print("OpenAI API Key:", openai_api_key)  # Add this line
    ...
    st.header("Chat with Signa G-AI:")
    ...
    with st.sidebar:
        ...
        # Display checkboxes for each uploaded PDF
        selected_files = []
        for i, pdf in enumerate(pdf_docs):
            checkbox = st.checkbox(f"File {i + 1}: {pdf.name}")
            if checkbox:
        ...
        if st.button("Process"):
            if selected_files:
                ...
                st.warning('Select Atleast One PDF')
